...
async def test_fast_read(dut):
    ...
    ret_val = await qspi.read_fast(dut.sio_i, dut.sio_o, dut.sio_oe, dut.sck_i, dut.sce_i, 0x83, freq=spi_freq, sce_pol=1, log=dut._log.info)
...
async def test_cmd_translation(dut):
    ...
    await qspi.erase_sect(dut.sio_i, dut.sck_i, dut.sce_i, 0x50000, freq=spi_freq, sce_pol=1, log=dut._log.info)
...
async def test_prog_word(dut):
    ...
    await qspi.prog_word(dut.sio_i, dut.sck_i, dut.sce_i, 0x30000, 0xABCD, freq=spi_freq, sce_pol=1, log=dut._log.info)
...
async def test_loopback(dut):
    ...
    ret = await qspi.loopback(dut.sio_i, dut.sio_o, dut.sio_oe, dut.sck_i, dut.sce_i, 0x11ABCD, freq=spi_freq, sce_pol=1, log=dut._log.info)
    assert ret == 0xABCD

    ret = await qspi.loopback(dut.sio_i, dut.sio_o, dut.sio_oe, dut.sck_i, dut.sce_i, 0x10, freq=6, sce_pol=1, log=dut._log.info)
    assert ret == 0x0010
...
async def test_fast_read_mc(dut):
    ...
    for i in range(100):
        ...
        toff = 8.0 * random()
        task = cocotb.start_soon(wb.slave_read_expect(bus_wb, 0x83, data=0x3456, timeout=2000, stall_cycles=4, log=dut._log.info))
        ret_val = await qspi.read_fast(dut.sio_i, dut.sio_o, dut.sio_oe, dut.sck_i, dut.sce_i, 0x83, freq=spi_freq, toff=toff, sce_pol=1)
        assert int(ret_val) == 0x3456
        await Join(task)
        await Timer(10, 'ns')
